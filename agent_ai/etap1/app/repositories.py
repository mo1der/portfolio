...
def get_ticket_timeline(db, ticket_id: int):
        ...
        timeline_items.append(
            {
                ...
                "title": f"Auto assigned to {ticket.assigned_to}",
                "description": (
                    "Ticket was automatically assigned based on category "
                    "and priority routing rules."
                ),
                "author": "SYSTEM",
                "created_at": ticket.created_at,
            }
        ...
        timeline_items.append(
            {
                ...
                "title": f"SLA created: {ticket.sla_status}",
                "description": f"SLA due at {ticket.sla_due_at.isoformat()}",
                "author": "SYSTEM",
                "created_at": ticket.created_at,
            }
        )
    ...
    if ticket.suggested_reply:
        reply_event_type = (
            "AI_REPLY_GENERATED"
            if ticket.suggested_reply_source == "AI"
            else "RULE_BASED_REPLY_GENERATED"
        )

        timeline_items.append(
            {
                "event_type": reply_event_type,
                "title": f"Suggested reply generated by {ticket.suggested_reply_source}",
                "description": ticket.suggested_reply,
                "author": "SYSTEM",
                "created_at": ticket.created_at,
            }
        )

    if ticket.possible_duplicate:
        timeline_items.append(
            {
                "event_type": "DUPLICATE_DETECTED",
                "title": f"Possible duplicate of ticket #{ticket.duplicate_ticket_id}",
                "description": f"Duplicate score: {ticket.duplicate_score}",
                "author": "SYSTEM",
                "created_at": ticket.created_at,
            }
        )
